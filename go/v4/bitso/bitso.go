...
func  (this *bitso) Describe() interface{}  {
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            "0201": ccxt.AuthenticationError,
            "104": ccxt.InvalidNonce,
            "0304": ccxt.BadRequest,
        },
    })
}
/**
...
func  (this *bitso) ParseLedgerEntry(item interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(referenceId, nil)) {
        referenceId = this.SafeString(details, "tid")
    }
    if ccxt.IsTrue(ccxt.IsEqual(operation, "funding")) {
        direction = "in"
    } else if ccxt.IsTrue(ccxt.IsEqual(operation, "withdrawal")) {
        direction = "out"
    } else if ccxt.IsTrue(ccxt.IsEqual(operation, "trade")) {
        direction = nil
    } else if ccxt.IsTrue(ccxt.IsEqual(operation, "fee")) {
        direction = "out"
        var cost interface{} = ccxt.Precise.StringAbs(amount)
...
func  (this *bitso) FetchMarkets(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(markets)); i++ {
                ...
                var base interface{} = ccxt.ToUpper(baseId)
                var quote interface{} = ccxt.ToUpper(quoteId)
                ...
                for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(feeTiers)); j++ {
                    var tier interface{} = ccxt.GetValue(feeTiers, j)
                    ...
                    ccxt.AppendToArray(&takerFees, []interface{}{volume, takerFee})
                    ccxt.AppendToArray(&makerFees, []interface{}{volume, makerFee})
                    if ccxt.IsTrue(ccxt.IsEqual(j, 0)) {
                ...
                ccxt.AddElementToObject(fee, "tiers", tiers)
                ...
                var defaultPricePrecision interface{} = this.SafeNumber(ccxt.GetValue(this.Options, "precision"), quote, ccxt.GetValue(this.Options, "defaultPrecision"))
                ccxt.AppendToArray(&result, this.Extend(map[string]interface{} {
                    "id": id,
                    ...
                    "optionType": nil,
                    "precision": map[string]interface{} {
                        "amount": this.SafeNumber(ccxt.GetValue(this.Options, "precision"), base, ccxt.GetValue(this.Options, "defaultPrecision")),
...
func  (this *bitso) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ch <- this.ParseOrderBook(orderbook, ccxt.GetValue(market, "symbol"), timestamp, "bids", "asks", "price", "amount")
            return nil
...
func  (this *bitso) FetchOHLCV(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ...
                if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                    ...
                    ccxt.AddElementToObject(request, "end", this.Sum(since, ccxt.Multiply(ccxt.Multiply(duration, limit), 1000)))
                }
            } else if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                var now interface{} = this.Milliseconds()
                ccxt.AddElementToObject(request, "end", now)
                ccxt.AddElementToObject(request, "start", ccxt.Subtract(now, ccxt.Multiply(ccxt.Multiply(this.ParseTimeframe(timeframe), 1000), limit)))
            }
...
func  (this *bitso) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(side, nil)) {
        if ccxt.IsTrue(ccxt.IsEqual(side, makerSide)) {
            takerOrMaker = "maker"
        } else {
            takerOrMaker = "taker"
        }
    } else {
        if ccxt.IsTrue(ccxt.IsEqual(makerSide, "buy")) {
            side = "sell"
        } else {
            side = "buy"
        }
...
func  (this *bitso) FetchTrades(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ch <- this.ParseTrades(ccxt.GetValue(response, "payload"), market, since, limit)
            return nil
...
func  (this *bitso) FetchMyTrades(optionalArgs ...interface{}) <- chan interface{} {
            ...
            since := ccxt.GetArg(optionalArgs, 1, nil)
            _ = since
            limit := ccxt.GetArg(optionalArgs, 2, 25)
            _ = limit
            params := ccxt.GetArg(optionalArgs, 3, map[string]interface{} {})
            _ = params
            ...
            var markerInParams interface{} =     (ccxt.InOp(params, "marker"))
            // warn the user with an exception if the user wants to filter
            // starting from since timestamp, but does not set the trade id with an extra 'marker' param
            if ccxt.IsTrue(ccxt.IsTrue((!ccxt.IsEqual(since, nil))) && !ccxt.IsTrue(markerInParams)) {
                panic(ccxt.ExchangeError(ccxt.Add(this.Id, " fetchMyTrades() does not support fetching trades starting from a timestamp with the `since` argument, use the `marker` extra param to filter starting from an integer trade id")))
            }
            // convert it to an integer unconditionally
            if ccxt.IsTrue(markerInParams) {
                params = this.Extend(params, map[string]interface{} {
                    "marker": ccxt.ParseInt(ccxt.GetValue(params, "marker")),
                })
            }
            var request interface{} = map[string]interface{} {
                "book": ccxt.GetValue(market, "id"),
...
func  (this *bitso) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                ...
                "side": side,
                "type": typeVar,
                "major": this.AmountToPrecision(ccxt.GetValue(market, "symbol"), amount),
            }
            ...
            ccxt.PanicOnError(response)
            var id interface{} = this.SafeString(ccxt.GetValue(response, "payload"), "oid")
...
func  (this *bitso) CancelOrders(ids interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            if !ccxt.IsTrue(ccxt.IsArray(ids)) {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " cancelOrders() ids argument should be an array")))
            }
            var market interface{} = nil
            if ccxt.IsTrue(!ccxt.IsEqual(symbol, nil)) {
                market = this.Market(symbol)
            }
            var oids interface{} = ccxt.Join(ids, ",")
            var request interface{} = map[string]interface{} {
                "oids": oids,
            }
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(payload)); i++ {
                var id interface{} = ccxt.GetValue(payload, i)
                ccxt.AppendToArray(&orders, this.ParseOrder(id, market))
            }
...
func  (this *bitso) CancelAllOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            _ = params
            if ccxt.IsTrue(!ccxt.IsEqual(symbol, nil)) {
                panic(ccxt.NotSupported(ccxt.Add(this.Id, " cancelAllOrders() deletes all orders for user, it does not support filtering by symbol.")))
            }
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(payload)); i++ {
                var order interface{} = this.ParseOrder(ccxt.GetValue(payload, i))
                ccxt.AppendToArray(&canceledOrders, order)
            }
...
func  (this *bitso) FetchOpenOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            // starting from since timestamp, but does not set the trade id with an extra 'marker' param
            if ccxt.IsTrue(ccxt.IsTrue((!ccxt.IsEqual(since, nil))) && !ccxt.IsTrue(markerInParams)) {
                panic(ccxt.ExchangeError(ccxt.Add(this.Id, " fetchOpenOrders() does not support fetching orders starting from a timestamp with the `since` argument, use the `marker` extra param to filter starting from an integer trade id")))
            }
            // convert it to an integer unconditionally
            if ccxt.IsTrue(markerInParams) {
                params = this.Extend(params, map[string]interface{} {
            ...
            ccxt.PanicOnError(response)
            var orders interface{} = this.ParseOrders(ccxt.GetValue(response, "payload"), market, since, limit)
...
func  (this *bitso) FetchOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsArray(payload)) {
                var numOrders interface{} =         ccxt.GetArrayLength(ccxt.GetValue(response, "payload"))
                if ccxt.IsTrue(ccxt.IsEqual(numOrders, 1)) {
        
                    ch <- this.ParseOrder(ccxt.GetValue(payload, 0))
                    return nil
                }
            }
...
func  (this *bitso) FetchOrderTrades(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ch <- this.ParseTrades(ccxt.GetValue(response, "payload"), market)
            return nil
...
func  (this *bitso) FetchDepositAddress(code interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                "fund_currency": ccxt.GetValue(currency, "id"),
            }
        
            response:= (<-this.PrivateGetFundingDestination(this.Extend(request, params)))
            ccxt.PanicOnError(response)
            var address interface{} = this.SafeString(ccxt.GetValue(response, "payload"), "account_identifier")
            var tag interface{} = nil
            if ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(address, "?dt="), 0)) {
                var parts interface{} = ccxt.Split(address, "?dt=")
...
func  (this *bitso) FetchTransactionFees(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(depositFees)); i++ {
                var depositFee interface{} = ccxt.GetValue(depositFees, i)
            ...
            var currencyIds interface{} = ccxt.ObjectKeys(withdrawalFees)
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(currencyIds)); i++ {
                ...
                ccxt.AddElementToObject(result, code, map[string]interface{} {
            "deposit": this.SafeValue(ccxt.GetValue(result, code), "deposit"),
            "withdraw": this.SafeNumber(withdrawalFees, currencyId),
            "info": map[string]interface{} {
                "deposit": this.SafeValue(ccxt.GetValue(ccxt.GetValue(result, code), "info"), "deposit"),
                "withdraw": this.SafeNumber(withdrawalFees, currencyId),
            },
        })
            }
...
func  (this *bitso) ParseDepositWithdrawFees(response interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(depositResponse)); i++ {
        var entry interface{} = ccxt.GetValue(depositResponse, i)
        var currencyId interface{} = this.SafeString(entry, "currency")
        var code interface{} = this.SafeCurrencyCode(currencyId)
        if ccxt.IsTrue(ccxt.IsTrue((ccxt.IsEqual(codes, nil))) || ccxt.IsTrue((ccxt.InOp(codes, code)))) {
            ccxt.AddElementToObject(result, code, map[string]interface{} {
    "deposit": map[string]interface{} {
        "fee": this.SafeNumber(entry, "fee"),
        "percentage": !ccxt.IsTrue(this.SafeValue(entry, "is_fixed")),
    },
    ...
    var withdrawalKeys interface{} = ccxt.ObjectKeys(withdrawalResponse)
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(withdrawalKeys)); i++ {
        var currencyId interface{} = ccxt.GetValue(withdrawalKeys, i)
        var code interface{} = this.SafeCurrencyCode(currencyId)
        if ccxt.IsTrue(ccxt.IsTrue((ccxt.IsEqual(codes, nil))) || ccxt.IsTrue((ccxt.InOp(codes, code)))) {
            var withdrawFee interface{} = this.ParseNumber(ccxt.GetValue(withdrawalResponse, currencyId))
            var resultValue interface{} = this.SafeValue(result, code)
            if ccxt.IsTrue(ccxt.IsEqual(resultValue, nil)) {
                ccxt.AddElementToObject(result, code, this.DepositWithdrawFee(map[string]interface{} {}))
            }
            ccxt.AddElementToObject(ccxt.GetValue(ccxt.GetValue(result, code), "withdraw"), "fee", withdrawFee)
            ccxt.AddElementToObject(ccxt.GetValue(ccxt.GetValue(result, code), "info"), code, withdrawFee)
        }
    }
    return result
}
...
func  (this *bitso) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            __tmp25917 := this.HandleWithdrawTagAndParams(tag, params)
            tag = ccxt.GetValue(__tmp25917, 0)
            params = ccxt.GetValue(__tmp25917, 1)
            this.CheckAddress(address)
        
            retRes17178 := (<-this.LoadMarkets())
            ccxt.PanicOnError(retRes17178)
            ...
            var currency interface{} = this.Currency(code)
            var method interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.InOp(methods, code))), ccxt.GetValue(methods, code), nil)
            if ccxt.IsTrue(ccxt.IsEqual(method, nil)) {
                panic(ccxt.ExchangeError(ccxt.Add(ccxt.Add(this.Id, " not valid withdraw coin: "), code)))
            }
            var request interface{} = map[string]interface{} {
                "amount": amount,
                "address": address,
            ...
            var classMethod interface{} = ccxt.Add(ccxt.Add("privatePost", method), "Withdrawal")
...
func  (this *bitso) ParseTransaction(transaction interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var networkCodeUpper interface{} = ccxt.Ternary(ccxt.IsTrue((!ccxt.IsEqual(networkCode, nil))), ccxt.ToUpper(networkCode), nil)
    return map[string]interface{} {
        ...
        "address": ccxt.Ternary(ccxt.IsTrue((!ccxt.IsEqual(withdrawalAddress, nil))), withdrawalAddress, receivingAddress),
        "addressTo": withdrawalAddress,
        "amount": this.SafeNumber(transaction, "amount"),
        "type": ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(withdrawId, nil))), "deposit", "withdrawal"),
...
func  (this *bitso) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    _ = body
    var endpoint interface{} = ccxt.Add(ccxt.Add(ccxt.Add("/", this.Version), "/"), this.ImplodeParams(path, params))
    ...
    if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(method, "GET")) || ccxt.IsTrue(ccxt.IsEqual(method, "DELETE"))) {
        if ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(query))) {
            endpoint = ccxt.Add(endpoint, ccxt.Add("?", this.Urlencode(query)))
        }
    }
    var url interface{} = ccxt.Add(ccxt.GetValue(ccxt.GetValue(this.Urls, "api"), "rest"), endpoint)
    if ccxt.IsTrue(ccxt.IsEqual(api, "private")) {
        this.CheckRequiredCredentials()
        var nonce interface{} = ccxt.ToString(this.Nonce())
        endpoint = ccxt.Add("/api", endpoint)
        var request interface{} = ccxt.Join([]interface{}{nonce, method, endpoint}, "")
        if ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(method, "GET")) && ccxt.IsTrue(!ccxt.IsEqual(method, "DELETE"))) {
            if ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(query))) {
                body = this.Json(query)
                request = ccxt.Add(request, body)
            }
        }
        var signature interface{} = this.Hmac(this.Encode(request), this.Encode(this.Secret), ccxt.Sha256)
        var auth interface{} = ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(this.ApiKey, ":"), nonce), ":"), signature)
        headers = map[string]interface{} {
            "Authorization": ccxt.Add("Bitso ", auth),
        }
    }
    return map[string]interface{} {
        "url": url,
...
func  (this *bitso) HandleErrors(httpCode interface{}, reason interface{}, url interface{}, method interface{}, headers interface{}, body interface{}, response interface{}, requestHeaders interface{}, requestBody interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.InOp(response, "success")) {
        //
