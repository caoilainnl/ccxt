...
func  (this *cryptomus) ParseMarket(market interface{}) interface{}  {
    //
    //     {
    //         "id": "01JHN5EFT64YC4HR9KCGM5M65D",
    //         "symbol": "POL_USDT",
...
func  (this *cryptomus) ParseTicker(ticker interface{}, optionalArgs ...interface{}) interface{}  {
    //
    //     {
...
func  (this *cryptomus) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                "currencyPair": ccxt.GetValue(market, "id"),
            }
            var level interface{} = 0
            __tmp340244 := this.HandleOptionAndParams(params, "fetchOrderBook", "level", level)
            level = ccxt.GetValue(__tmp340244, 0)
            params = ccxt.GetValue(__tmp340244, 1)
...
func  (this *cryptomus) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    //
    //     {
...
func  (this *cryptomus) ParseBalance(balance interface{}) interface{}  {
    //
    //     {
    //         "ticker": "AVAX",
    //         "available": "0.00000000",
    ...
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(balance)); i++ {
        var balanceEntry interface{} = ccxt.GetValue(balance, i)
        ...
        ccxt.AddElementToObject(account, "used", this.SafeString(balanceEntry, "held"))
...
func  (this *cryptomus) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var sideBuy interface{} = ccxt.IsEqual(side, "buy")
            var amountToString interface{} = this.NumberToString(amount)
            var priceToString interface{} = this.NumberToString(price)
            var cost interface{} = nil
            __tmp190235 := this.HandleParamString(params, "cost")
            cost = ccxt.GetValue(__tmp190235, 0)
            params = ccxt.GetValue(__tmp190235, 1)
            var response interface{} = nil
            if ccxt.IsTrue(ccxt.IsEqual(typeVar, "market")) {
                if ccxt.IsTrue(sideBuy) {
                    var createMarketBuyOrderRequiresPrice interface{} = true
                    __tmp221036 := this.HandleOptionAndParams(params, "createOrder", "createMarketBuyOrderRequiresPrice", true)
                    createMarketBuyOrderRequiresPrice = ccxt.GetValue(__tmp221036, 0)
                    params = ccxt.GetValue(__tmp221036, 1)
                    if ccxt.IsTrue(createMarketBuyOrderRequiresPrice) {
                        if ccxt.IsTrue(ccxt.IsTrue((ccxt.IsEqual(price, nil))) && ccxt.IsTrue((ccxt.IsEqual(cost, nil)))) {
                            panic(ccxt.InvalidOrder(ccxt.Add(this.Id, " createOrder() requires the price argument for market buy orders to calculate the total cost to spend (amount * price), alternatively set the createMarketBuyOrderRequiresPrice option of param to false and pass the cost to spend in the amount argument")))
                        } else if ccxt.IsTrue(ccxt.IsEqual(cost, nil)) {
                            cost = ccxt.Precise.StringMul(amountToString, priceToString)
                        }
                    } else {
                        cost = ccxt.Ternary(ccxt.IsTrue(cost), cost, amountToString)
                    }
                    ccxt.AddElementToObject(request, "value", cost)
                } else {
                    ccxt.AddElementToObject(request, "quantity", amountToString)
                }
                ...
                if ccxt.IsTrue(ccxt.IsEqual(price, nil)) {
                    panic(ccxt.ArgumentsRequired(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " createOrder() requires a price parameter for a "), typeVar), " order")))
                }
                ccxt.AddElementToObject(request, "quantity", amountToString)
                ccxt.AddElementToObject(request, "price", price)
                response = (<-this.PrivatePostV2UserApiExchangeOrders(this.Extend(request, params)))
            } else {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " createOrder() requires a type parameter (limit or market)")))
            }
...
func  (this *cryptomus) FetchCanceledAndClosedOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(result)); i++ {
                var order interface{} = ccxt.GetValue(result, i)
                ccxt.AppendToArray(&orders, this.ParseOrder(order, market))
            }
...
func  (this *cryptomus) ParseOrder(order interface{}, optionalArgs ...interface{}) interface{}  {
    //
    // createOrder
    //     {
    //         "order_id": "01JEXAFCCC5ZVJPZAAHHDKQBNG"
...
func  (this *cryptomus) ParseOrderStatus(optionalArgs ...interface{}) interface{}  {
    status := ccxt.GetArg(optionalArgs, 0, nil)
    _ = status
...
func  (this *cryptomus) ParseFeeTiers(feeTiers interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(feeTiers)); i++ {
        var tier interface{} = ccxt.GetValue(feeTiers, i)
        ...
        maker = ccxt.Precise.StringDiv(maker, "100")
        taker = ccxt.Precise.StringDiv(taker, "100")
        ccxt.AppendToArray(&makerFees, []interface{}{turnover, this.ParseNumber(maker)})
        ccxt.AppendToArray(&takerFees, []interface{}{turnover, this.ParseNumber(taker)})
    }
...
func  (this *cryptomus) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var url interface{} = ccxt.Add(ccxt.Add(ccxt.GetValue(ccxt.GetValue(this.Urls, "api"), api), "/"), endpoint)
    if ccxt.IsTrue(ccxt.IsEqual(api, "private")) {
        ...
        var stringToSign interface{} = ccxt.Add(jsonParamsBase64, this.Secret)
        var signature interface{} = this.Hash(this.Encode(stringToSign), ccxt.Md5)
        ccxt.AddElementToObject(headers, "sign", signature)
    } else {
        var query interface{} = this.Urlencode(params)
        if ccxt.IsTrue(!ccxt.IsEqual(ccxt.GetArrayLength(query), 0)) {
            url = ccxt.Add(url, ccxt.Add("?", query))
...
func  (this *cryptomus) HandleErrors(httpCode interface{}, reason interface{}, url interface{}, method interface{}, headers interface{}, body interface{}, response interface{}, requestHeaders interface{}, requestBody interface{}) interface{}  {
    ...
    } else if ccxt.IsTrue(ccxt.InOp(response, "message")) {
        //
