    ...
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "api": map[string]interface{} {
            ...
            "private": map[string]interface{} {
                ...
                "post": map[string]interface{} {
                    "orders/{pair}": ccxt.Divide(1, 2),
                    "orders/batch": ccxt.Divide(20, 3),
                ...
                "delete": map[string]interface{} {
                    "orders/{pair}/{id}": ccxt.Divide(2, 3),
                    "orders/all": 5,
                    "orders/{pair}": 5,
                },
            },
        ...
        "exceptions": map[string]interface{} {
            "exact": map[string]interface{} {
                "Unsupported currency.": ccxt.BadRequest,
                "Unsupported order type": ccxt.BadRequest,
                "Invalid body": ccxt.BadRequest,
                "Invalid Signature": ccxt.AuthenticationError,
                "Address not in whitelist.": ccxt.BadRequest,
            },
            "broad": map[string]interface{} {
                "Invalid amount": ccxt.InvalidOrder,
                "ccxt.Balance for ": ccxt.InsufficientFunds,
                "Invalid ": ccxt.BadRequest,
                "Wrong parameter": ccxt.BadRequest,
            },
        },
...
func  (this *bitopro) FetchCurrencies(optionalArgs ...interface{}) <- chan interface{} {
            ...
            "name": nil,
            "active": ccxt.IsTrue(deposit) && ccxt.IsTrue(withdraw),
            "deposit": deposit,
            "withdraw": withdraw,
            "fee": fee,
            "precision": nil,
...
func  (this *bitopro) ParseMarket(market interface{}) interface{}  {
    var active interface{} =     !ccxt.IsTrue(this.SafeBool(market, "maintain"))
    var id interface{} = this.SafeString(market, "pair")
    var uppercaseId interface{} = ccxt.ToUpper(id)
...
func  (this *bitopro) FetchOHLCV(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsEqual(limit, nil)) {
                ...
                limit = ccxt.MathMin(limit, 75000) // supports slightly more than 75k candles atm, but limit here to avoid errors
            }
            ...
            if ccxt.IsTrue(ccxt.IsEqual(since, nil)) {
                ...
                ccxt.AddElementToObject(request, "from", ccxt.Subtract(ccxt.GetValue(request, "to"), (ccxt.Multiply(limit, timeframeInSeconds))))
            } else {
                var timeframeInMilliseconds interface{} = ccxt.Multiply(timeframeInSeconds, 1000)
                alignedSince = ccxt.Multiply(ccxt.MathFloor(ccxt.Divide(since, timeframeInMilliseconds)), timeframeInMilliseconds)
                ccxt.AddElementToObject(request, "from", ccxt.MathFloor(ccxt.Divide(since, 1000)))
                ccxt.AddElementToObject(request, "to", this.Sum(ccxt.GetValue(request, "from"), ccxt.Multiply(limit, timeframeInSeconds)))
            }
...
func  (this *bitopro) InsertMissingCandles(candles interface{}, distance interface{}, since interface{}, limit interface{}) interface{}  {
    ...
    var length interface{} =     ccxt.GetArrayLength(candles)
    if ccxt.IsTrue(ccxt.IsEqual(length, 0)) {
        return candles
    }
    var result interface{} = []interface{}{}
    var copyFrom interface{} = ccxt.GetValue(candles, 0)
    var timestamp interface{} = nil
    if ccxt.IsTrue(ccxt.IsEqual(since, nil)) {
        timestamp = ccxt.GetValue(copyFrom, 0)
    } else {
        timestamp = since
    }
    var i interface{} = 0
    var candleLength interface{} =     ccxt.GetArrayLength(candles)
    var resultLength interface{} = 0
    for ccxt.IsTrue((ccxt.IsLessThan(resultLength, limit))) && ccxt.IsTrue((ccxt.IsLessThan(i, candleLength))) {
        var candle interface{} = ccxt.GetValue(candles, i)
        if ccxt.IsTrue(ccxt.IsEqual(ccxt.GetValue(candle, 0), timestamp)) {
            ccxt.AppendToArray(&result, candle)
            i = this.Sum(i, 1)
        } else {
            var copy interface{} = this.ArrayConcat([]interface{}{}, copyFrom)
            ccxt.AddElementToObject(copy, 0, timestamp)
            // set open, high, low to close
            ccxt.AddElementToObject(copy, 1, ccxt.GetValue(copy, 4))
            ccxt.AddElementToObject(copy, 2, ccxt.GetValue(copy, 4))
            ccxt.AddElementToObject(copy, 3, ccxt.GetValue(copy, 4))
            ccxt.AddElementToObject(copy, 5, this.ParseNumber("0"))
            ccxt.AppendToArray(&result, copy)
        }
        timestamp = this.Sum(timestamp, ccxt.Multiply(distance, 1000))
        resultLength = ccxt.GetArrayLength(result)
        copyFrom = ccxt.GetValue(result, ccxt.Subtract(resultLength, 1))
    }
    return result
}
...
func  (this *bitopro) ParseOrder(order interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.Precise.StringGt(feeAmount, "0")) {
        fee = map[string]interface{} {
            "currency": feeSymbol,
            "cost": feeAmount,
        }
...
func  (this *bitopro) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsEqual(orderType, "LIMIT")) {
            ...
            if ccxt.IsTrue(ccxt.IsEqual(orderType, "STOP_LIMIT")) {
                ...
                if ccxt.IsTrue(ccxt.IsEqual(triggerPrice, nil)) {
                    panic(ccxt.InvalidOrder(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " createOrder() requires a triggerPrice parameter for "), orderType), " orders")))
                } else {
                ...
                if ccxt.IsTrue(ccxt.IsEqual(condition, nil)) {
                    panic(ccxt.InvalidOrder(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " createOrder() requires a condition parameter for "), orderType), " orders")))
                } else {
                    ccxt.AddElementToObject(request, "condition", condition)
                }
            }
            var postOnly interface{} = this.IsPostOnly(ccxt.IsEqual(orderType, "MARKET"), nil, params)
            if ccxt.IsTrue(postOnly) {
                ccxt.AddElementToObject(request, "timeInForce", "POST_ONLY")
            }
...
func  (this *bitopro) ParseCancelOrders(data interface{}) interface{}  {
    var dataKeys interface{} = ccxt.ObjectKeys(data)
    var orders interface{} = []interface{}{}
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(dataKeys)); i++ {
        var marketId interface{} = ccxt.GetValue(dataKeys, i)
        var orderIds interface{} = ccxt.GetValue(data, marketId)
        for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(orderIds)); j++ {
            ccxt.AppendToArray(&orders, this.SafeOrder(map[string]interface{} {
                "info": ccxt.GetValue(orderIds, j),
                "id": ccxt.GetValue(orderIds, j),
...
func  (this *bitopro) CancelOrders(ids interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var market interface{} = this.Market(symbol)
            var id interface{} = ccxt.GetValue(market, "uppercaseId")
            var request interface{} = map[string]interface{} {}
            ccxt.AddElementToObject(request, id, ids)
...
func  (this *bitopro) FetchOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ccxt.AddElementToObject(request, "startTimestamp", since)
            }
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "limit", limit)
            }
...
func  (this *bitopro) ParseTransaction(transaction interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(networkId, "MAIN")) {
        networkId = code
    }
    return map[string]interface{} {
        "info": transaction,
...
func  (this *bitopro) FetchDeposits(optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsEqual(code, nil)) {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " fetchDeposits() requires the code argument")))
            }
...
func  (this *bitopro) FetchWithdrawals(optionalArgs ...interface{}) <- chan interface{} {
            ...
            _ = params
            if ccxt.IsTrue(ccxt.IsEqual(code, nil)) {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " fetchWithdrawals() requires the code argument")))
            }
        
            retRes16758 := (<-this.LoadMarkets())
            ccxt.PanicOnError(retRes16758)
            var currency interface{} = this.SafeCurrency(code)
            var request interface{} = map[string]interface{} {
                "currency": ccxt.GetValue(currency, "id"),
            }
...
func  (this *bitopro) FetchWithdrawal(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            _ = params
            if ccxt.IsTrue(ccxt.IsEqual(code, nil)) {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " fetchWithdrawal() requires the code argument")))
            }
...
func  (this *bitopro) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            __tmp992817 := this.HandleWithdrawTagAndParams(tag, params)
            tag = ccxt.GetValue(__tmp992817, 0)
            params = ccxt.GetValue(__tmp992817, 1)
        
            retRes17678 := (<-this.LoadMarkets())
            ccxt.PanicOnError(retRes17678)
            ...
            if ccxt.IsTrue(ccxt.InOp(params, "network")) {
                ...
                if ccxt.IsTrue(ccxt.IsEqual(networkId, nil)) {
                    panic(ccxt.ExchangeError(ccxt.Add(ccxt.Add(this.Id, " invalid network "), requestedNetwork)))
                }
                ccxt.AddElementToObject(request, "protocol", networkId)
            }
            if ccxt.IsTrue(!ccxt.IsEqual(tag, nil)) {
                ccxt.AddElementToObject(request, "message", tag)
            }
...
func  (this *bitopro) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    ccxt.AddElementToObject(headers, "X-BITOPRO-API", "ccxt")
    if ccxt.IsTrue(ccxt.IsEqual(api, "private")) {
        ...
        if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(method, "POST")) || ccxt.IsTrue(ccxt.IsEqual(method, "PUT"))) {
            ...
            ccxt.AddElementToObject(headers, "X-BITOPRO-APIKEY", this.ApiKey)
            ccxt.AddElementToObject(headers, "X-BITOPRO-PAYLOAD", payload)
            ccxt.AddElementToObject(headers, "X-BITOPRO-SIGNATURE", signature)
        } else if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(method, "GET")) || ccxt.IsTrue(ccxt.IsEqual(method, "DELETE"))) {
    ...
    } else if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(api, "public")) && ccxt.IsTrue(ccxt.IsEqual(method, "GET"))) {
        if ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(query))) {
            url = ccxt.Add(url, ccxt.Add("?", this.Urlencode(query)))
        }
    }
...
func  (this *bitopro) HandleErrors(code interface{}, reason interface{}, url interface{}, method interface{}, headers interface{}, body interface{}, response interface{}, requestHeaders interface{}, requestBody interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(code, 200)) && ccxt.IsTrue(ccxt.IsLessThan(code, 300))) {
        return nil
    }
