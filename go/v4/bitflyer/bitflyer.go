    ...
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            "exact": map[string]interface{} {
                "-2": ccxt.OnMaintenance,
            },
        },
    })
}
func  (this *bitflyer) ParseExpiryDate(expiry interface{}) interface{}  {
    var day interface{} = ccxt.Slice(expiry, 0, 2)
    var monthName interface{} = ccxt.Slice(expiry, 2, 5)
    var year interface{} = ccxt.Slice(expiry, 5, 9)
    ...
    return this.Parse8601(ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(year, "-"), month), "-"), day), "T00:00:00Z"))
}
...
func  (this *bitflyer) FetchMarkets(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(markets)); i++ {
                ...
                var currencies interface{} = ccxt.Split(id, "_")
                var marketType interface{} = this.SafeString(market, "market_type")
                var swap interface{} =         (ccxt.IsEqual(marketType, "FX"))
                var future interface{} =         (ccxt.IsEqual(marketType, "Futures"))
                var spot interface{} = !ccxt.IsTrue(swap) && !ccxt.IsTrue(future)
                ...
                if ccxt.IsTrue(spot) {
                    ...
                    if ccxt.IsTrue(ccxt.IsEqual(alias, nil)) {
                        ...
                        var expiryDate interface{} = ccxt.Slice(id, ccxt.OpNeg(9), nil)
                        expiry = this.ParseExpiryDate(expiryDate)
                    } else {
                        var splitAlias interface{} = ccxt.Split(alias, "_")
                        var currencyIds interface{} = this.SafeString(splitAlias, 0)
                        baseId = ccxt.Slice(currencyIds, 0, ccxt.OpNeg(3))
                        quoteId = ccxt.Slice(currencyIds, ccxt.OpNeg(3), nil)
                        var splitId interface{} = ccxt.Split(id, currencyIds)
                ...
                var taker interface{} = ccxt.GetValue(ccxt.GetValue(this.Fees, "trading"), "taker")
                var maker interface{} = ccxt.GetValue(ccxt.GetValue(this.Fees, "trading"), "maker")
                ...
                ccxt.AppendToArray(&result, map[string]interface{} {
                    ...
                    "active": true,
                    "contract": contract,
                    "linear": ccxt.Ternary(ccxt.IsTrue(spot), nil, true),
                    "inverse": ccxt.Ternary(ccxt.IsTrue(spot), nil, false),
                    "taker": taker,
                    "maker": maker,
                    "contractSize": nil,
                    "expiry": expiry,
...
func  (this *bitflyer) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                "product_code": ccxt.GetValue(market, "id"),
            }
        
            orderbook:= (<-this.PublicGetGetboard(this.Extend(request, params)))
            ccxt.PanicOnError(orderbook)
        
            ch <- this.ParseOrderBook(orderbook, ccxt.GetValue(market, "symbol"), nil, "bids", "asks", "price", "size")
            return nil
...
func  (this *bitflyer) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(side, nil)) {
        if ccxt.IsTrue(ccxt.IsLessThan(ccxt.GetLength(side), 1)) {
            side = nil
        }
    }
    var order interface{} = nil
    if ccxt.IsTrue(!ccxt.IsEqual(side, nil)) {
        var idInner interface{} = ccxt.Add(side, "_child_order_acceptance_id")
        if ccxt.IsTrue(ccxt.InOp(trade, idInner)) {
            order = ccxt.GetValue(trade, idInner)
        }
    }
...
func  (this *bitflyer) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                ...
                "child_order_type": ccxt.ToUpper(typeVar),
...
func  (this *bitflyer) FetchOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.InOp(ordersById, id)) {
        
                ch <- ccxt.GetValue(ordersById, id)
                return nil
            }
            panic(ccxt.OrderNotFound(ccxt.Add(ccxt.Add(this.Id, " No order found with id "), id)))
...
func  (this *bitflyer) FetchPositions(optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsEqual(symbols, nil)) {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " fetchPositions() requires a `symbols` argument, exactly one symbol in an array")))
            }
...
func  (this *bitflyer) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(code, "JPY")) && ccxt.IsTrue(!ccxt.IsEqual(code, "USD"))) && ccxt.IsTrue(!ccxt.IsEqual(code, "EUR"))) {
                panic(ccxt.ExchangeError(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " allows withdrawing JPY, USD, EUR only, "), code), " is not supported")))
            }
            var currency interface{} = this.Currency(code)
            var request interface{} = map[string]interface{} {
                "currency_code": ccxt.GetValue(currency, "id"),
                "amount": amount,
            }
...
func  (this *bitflyer) FetchDeposits(optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "count", limit) // default 100
            }
...
func  (this *bitflyer) ParseTransaction(transaction interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var fee interface{} = nil
    if ccxt.IsTrue(ccxt.InOp(transaction, "fee")) {
        typeVar = "withdrawal"
        ...
        fee = map[string]interface{} {
            "currency": code,
            "cost": this.ParseNumber(ccxt.Precise.StringAdd(feeCost, additionalFee)),
        }
    } else {
        typeVar = "deposit"
...
func  (this *bitflyer) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    _ = body
    var request interface{} = ccxt.Add(ccxt.Add("/", this.Version), "/")
    if ccxt.IsTrue(ccxt.IsEqual(api, "private")) {
        request = ccxt.Add(request, "me/")
    }
    request = ccxt.Add(request, path)
    ...
    var baseUrl interface{} = this.ImplodeHostname(ccxt.GetValue(ccxt.GetValue(this.Urls, "api"), "rest"))
    var url interface{} = ccxt.Add(baseUrl, request)
    if ccxt.IsTrue(ccxt.IsEqual(api, "private")) {
        this.CheckRequiredCredentials()
        var nonce interface{} = ccxt.ToString(this.Nonce())
        var auth interface{} = ccxt.Join([]interface{}{nonce, method, request}, "")
        if ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(params))) {
            if ccxt.IsTrue(!ccxt.IsEqual(method, "GET")) {
        ...
        headers = map[string]interface{} {
            ...
            "ACCESS-SIGN": this.Hmac(this.Encode(auth), this.Encode(this.Secret), ccxt.Sha256),
...
func  (this *bitflyer) HandleErrors(code interface{}, reason interface{}, url interface{}, method interface{}, headers interface{}, body interface{}, response interface{}, requestHeaders interface{}, requestBody interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(errorMessage, nil)) {
        this.ThrowExactlyMatchedException(ccxt.GetValue(this.Exceptions, "exact"), statusCode, feedback)
