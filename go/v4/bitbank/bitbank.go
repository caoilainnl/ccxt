...
func  (this *bitbank) ParseBalance(response interface{}) interface{}  {
    ...
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(assets)); i++ {
        var balance interface{} = ccxt.GetValue(assets, i)
        var currencyId interface{} = this.SafeString(balance, "asset")
        var code interface{} = this.SafeCurrencyCode(currencyId)
        var account interface{} = this.Account()
        ccxt.AddElementToObject(account, "free", this.SafeString(balance, "free_amount"))
        ccxt.AddElementToObject(account, "used", this.SafeString(balance, "locked_amount"))
        ccxt.AddElementToObject(account, "total", this.SafeString(balance, "onhand_amount"))
...
func  (this *bitbank) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            __tmp300434 := this.HandleWithdrawTagAndParams(tag, params)
            tag = ccxt.GetValue(__tmp300434, 0)
            params = ccxt.GetValue(__tmp300434, 1)
            if !ccxt.IsTrue((ccxt.InOp(params, "uuid"))) {
                panic(ccxt.ExchangeError(ccxt.Add(this.Id, " uuid is required for withdrawal")))
            }
...
func  (this *bitbank) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var url interface{} = ccxt.Add(this.ImplodeHostname(ccxt.GetValue(ccxt.GetValue(this.Urls, "api"), api)), "/")
    if ccxt.IsTrue(ccxt.IsTrue((ccxt.IsEqual(api, "public"))) || ccxt.IsTrue((ccxt.IsEqual(api, "markets")))) {
        ...
        url = ccxt.Add(url, ccxt.Add(ccxt.Add(this.Version, "/"), this.ImplodeParams(path, params)))
        if ccxt.IsTrue(ccxt.IsEqual(method, "POST")) {
            body = this.Json(query)
            auth = ccxt.Add(auth, body)
        } else {
            auth = ccxt.Add(auth, ccxt.Add(ccxt.Add(ccxt.Add("/", this.Version), "/"), path))
            if ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(query))) {
...
func  (this *bitbank) HandleErrors(httpCode interface{}, reason interface{}, url interface{}, method interface{}, headers interface{}, body interface{}, response interface{}, requestHeaders interface{}, requestBody interface{}) interface{}  {
    ...
    var data interface{} = this.SafeValue(response, "data")
    if ccxt.IsTrue(!ccxt.IsTrue(success) || !ccxt.IsTrue(data)) {
        var errorMessages interface{} = map[string]interface{} {
            ...
            "30013": "ccxt.Trade Please specify either",
            ...
            "70005": "ccxt.Order can not be accepted because purchase order is currently suspended",
            "70006": "We can not accept orders because we are currently unsubscribed ",
            "70009": "We are currently temporarily restricting orders to be carried out. Please use the limit order.",
            "70010": "We are temporarily raising the minimum order quantity as the system load is now rising.",
        }
        var code interface{} = this.SafeString(data, "code")
        var message interface{} = this.SafeString(errorMessages, code, "ccxt.Error")
        this.ThrowExactlyMatchedException(ccxt.GetValue(this.Exceptions, "exact"), code, message)
