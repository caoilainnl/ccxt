...
func  (this *bithumb) Describe() interface{}  {
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            ...
            "Unknown ccxt.Error": ccxt.ExchangeError,
            "After May 23th, recent_transactions is no longer, hence users will not be able to connect to recent_transactions": ccxt.ExchangeError,
        },
...
func  (this *bithumb) FetchMarkets(optionalArgs ...interface{}) <- chan interface{} {
            ...
            var quotes interface{} = ccxt.ObjectKeys(quoteCurrencies)
            var promises interface{} = []interface{}{}
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(quotes)); i++ {
                var request interface{} = map[string]interface{} {
                    "quoteId": ccxt.GetValue(quotes, i),
                }
                ccxt.AppendToArray(&promises, this.PublicGetTickerALLQuoteId(this.Extend(request, params)))
            }
        
            results:= (<-ccxt.PromiseAll(promises))
            ccxt.PanicOnError(results)
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(quotes)); i++ {
                var quote interface{} = ccxt.GetValue(quotes, i)
                var quoteId interface{} = quote
                var response interface{} = ccxt.GetValue(results, i)
                ...
                for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(currencyIds)); j++ {
                    var currencyId interface{} = ccxt.GetValue(currencyIds, j)
                    if ccxt.IsTrue(ccxt.IsEqual(currencyId, "date")) {
                        continue
                    }
                    var market interface{} = ccxt.GetValue(data, currencyId)
                    var base interface{} = this.SafeCurrencyCode(currencyId)
                    var active interface{} = true
                    if ccxt.IsTrue(ccxt.IsArray(market)) {
                        var numElements interface{} =                 ccxt.GetArrayLength(market)
                    ...
                    var entry interface{} = this.DeepExtend(map[string]interface{} {
                        ...
                        "precision": map[string]interface{} {
                            "amount": ccxt.ParseInt("4"),
                            "price": ccxt.ParseInt("4"),
                        },
                        "limits": map[string]interface{} {
...
func  (this *bithumb) ParseBalance(response interface{}) interface{}  {
    ...
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(codes)); i++ {
        ...
        ccxt.AddElementToObject(account, "total", this.SafeString(balances, ccxt.Add("total_", lowerCurrencyId)))
        ccxt.AddElementToObject(account, "used", this.SafeString(balances, ccxt.Add("in_use_", lowerCurrencyId)))
        ccxt.AddElementToObject(account, "free", this.SafeString(balances, ccxt.Add("available_", lowerCurrencyId)))
...
func  (this *bithumb) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "count", limit) // default 30, max 30
            }
...
func  (this *bithumb) FetchTickers(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(quotes)); i++ {
                var quote interface{} = ccxt.GetValue(quotes, i)
                var response interface{} = ccxt.GetValue(responses, i)
                //
                //     {
                //         "status":"0000",
                //         "data":{
                ...
                var currencyIds interface{} = ccxt.ObjectKeys(tickers)
                for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(currencyIds)); j++ {
                    var currencyId interface{} = ccxt.GetValue(currencyIds, j)
                    var ticker interface{} = ccxt.GetValue(data, currencyId)
                    var base interface{} = this.SafeCurrencyCode(currencyId)
                    var symbol interface{} = ccxt.Add(ccxt.Add(base, "/"), quote)
                    var market interface{} = this.SafeMarket(symbol)
                    ccxt.AddElementToObject(ticker, "date", timestamp)
...
func  (this *bithumb) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(transactionDatetime, nil)) {
        var parts interface{} = ccxt.Split(transactionDatetime, " ")
        var numParts interface{} =         ccxt.GetArrayLength(parts)
        if ccxt.IsTrue(ccxt.IsGreaterThan(numParts, 1)) {
            var transactionDate interface{} = ccxt.GetValue(parts, 0)
            var transactionTime interface{} = ccxt.GetValue(parts, 1)
            if ccxt.IsTrue(ccxt.IsLessThan(ccxt.GetLength(transactionTime), 8)) {
                transactionTime = ccxt.Add("0", transactionTime)
            }
            timestamp = this.Parse8601(ccxt.Add(ccxt.Add(transactionDate, " "), transactionTime))
        } else {
            timestamp = this.SafeIntegerProduct(trade, "transaction_date", 0.001)
        }
    }
    if ccxt.IsTrue(!ccxt.IsEqual(timestamp, nil)) {
        timestamp = ccxt.Subtract(timestamp, ccxt.Multiply(9, 3600000)) // they report UTC + 9 hours, server in Korean timezone
    }
    var typeVar interface{} = nil
    var side interface{} = this.SafeString(trade, "type")
    side = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(side, "ask"))), "sell", "buy")
...
func  (this *bithumb) FetchTrades(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "count", limit) // default 20, max 100
            }
...
func  (this *bithumb) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var market interface{} = this.Market(symbol)
            var request interface{} = map[string]interface{} {
                "order_currency": ccxt.GetValue(market, "id"),
                "payment_currency": ccxt.GetValue(market, "quote"),
                "units": amount,
            }
            ...
            if ccxt.IsTrue(ccxt.IsEqual(typeVar, "limit")) {
                ...
                method = ccxt.Add("privatePostTradeMarket", this.Capitalize(side))
            }
            ...
            if ccxt.IsTrue(ccxt.IsEqual(id, nil)) {
                panic(ccxt.InvalidOrder(ccxt.Add(this.Id, " createOrder() did not return an order id")))
            }
...
func  (this *bithumb) FetchOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                "order_id": id,
                "count": 1,
                "order_currency": ccxt.GetValue(market, "base"),
                "payment_currency": ccxt.GetValue(market, "quote"),
            }
...
func  (this *bithumb) ParseOrder(order interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    //         "type": "bid",
    //         "order_status": "Completed", // Completed, ccxt.Cancel ...
    //         "order_currency": "BTC",
    //         "payment_currency": "KRW",
    //         "watch_price": "0", // present in ccxt.Cancel order
    //         "order_price": "8601000",
    //         "order_qty": "0.007",
    //         "cancel_date": "", // filled in ccxt.Cancel order
    //         "cancel_type": "", // filled in ccxt.Cancel order, i.e. 사용자취소
    //         "contract": [
    //             {
    //                 "transaction_date": "1572497603902030",
    //                 "price": "8601000",
    ...
    var side interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(sideProperty, "bid"))), "buy", "sell")
    ...
    if ccxt.IsTrue(ccxt.Precise.StringEquals(price, "0")) {
        typeVar = "market"
    }
...
func  (this *bithumb) CancelOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var side_in_params interface{} =     (ccxt.InOp(params, "side"))
            if !ccxt.IsTrue(side_in_params) {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " cancelOrder() requires a `side` parameter (sell or buy)")))
            }
            var market interface{} = this.Market(symbol)
            var side interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(ccxt.GetValue(params, "side"), "buy"))), "bid", "ask")
...
func  (this *bithumb) CancelUnifiedOrder(order interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            _ = params
            var request interface{} = map[string]interface{} {
                "side": ccxt.GetValue(order, "side"),
            }
        
                retRes113515 :=  (<-this.CancelOrder(ccxt.GetValue(order, "id"), ccxt.GetValue(order, "symbol"), this.Extend(request, params)))
...
func  (this *bithumb) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            __tmp275092 := this.HandleWithdrawTagAndParams(tag, params)
            tag = ccxt.GetValue(__tmp275092, 0)
            params = ccxt.GetValue(__tmp275092, 1)
            ...
            if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsTrue(ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(code, "XRP")) || ccxt.IsTrue(ccxt.IsEqual(code, "XMR"))) || ccxt.IsTrue(ccxt.IsEqual(code, "EOS"))) || ccxt.IsTrue(ccxt.IsEqual(code, "STEEM"))) || ccxt.IsTrue(ccxt.IsEqual(code, "TON"))) {
                var destination interface{} = this.SafeString(params, "destination")
                if ccxt.IsTrue(ccxt.IsTrue((ccxt.IsEqual(tag, nil))) && ccxt.IsTrue((ccxt.IsEqual(destination, nil)))) {
                    panic(ccxt.ArgumentsRequired(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " "), code), " withdraw() requires a tag argument or an extra destination param")))
                } else if ccxt.IsTrue(!ccxt.IsEqual(tag, nil)) {
                    ccxt.AddElementToObject(request, "destination", tag)
                }
            }
...
func  (this *bithumb) FixCommaNumber(numberStr interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(numberStr, nil)) {
        return nil
    }
    var finalNumberStr interface{} = numberStr
    for ccxt.IsGreaterThan(ccxt.GetIndexOf(finalNumberStr, ","), ccxt.OpNeg(1)) {
        finalNumberStr = ccxt.Replace(finalNumberStr, ",", "")
    }
...
func  (this *bithumb) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var url interface{} = ccxt.Add(this.ImplodeHostname(ccxt.GetValue(ccxt.GetValue(this.Urls, "api"), api)), endpoint)
    ...
    if ccxt.IsTrue(ccxt.IsEqual(api, "public")) {
        ...
        var auth interface{} = ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(endpoint, "//" + "0"), body), "//" + "0"), nonce) // eslint-disable-line quotes
...
func  (this *bithumb) HandleErrors(httpCode interface{}, reason interface{}, url interface{}, method interface{}, headers interface{}, body interface{}, response interface{}, requestHeaders interface{}, requestBody interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.InOp(response, "status")) {
        ...
        if ccxt.IsTrue(!ccxt.IsEqual(status, nil)) {
            if ccxt.IsTrue(ccxt.IsEqual(status, "0000")) {
                return nil  // no error
            } else if ccxt.IsTrue(ccxt.IsEqual(message, "거래 진행중인 내역이 존재하지 않습니다.")) {
