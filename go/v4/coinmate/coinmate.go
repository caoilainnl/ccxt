    ...
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            ...
            "broad": map[string]interface{} {
                ...
                "Minimum ccxt.Order Size ": ccxt.InvalidOrder,
                "max allowed precision": ccxt.InvalidOrder,
                "TOO MANY REQUESTS": ccxt.RateLimitExceeded,
                "Access denied.": ccxt.AuthenticationError,
            },
        },
...
func  (this *coinmate) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(response)
            var orderbook interface{} = ccxt.GetValue(response, "data")
...
func  (this *coinmate) FetchTickers(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(keys)); i++ {
                var market interface{} = this.Market(ccxt.GetValue(keys, i))
                var ticker interface{} = this.ParseTicker(this.SafeValue(data, ccxt.GetValue(keys, i)), market)
                ccxt.AddElementToObject(result, ccxt.GetValue(market, "symbol"), ticker)
            }
...
func  (this *coinmate) FetchDepositsWithdrawals(optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ccxt.AddElementToObject(request, "timestampFrom", since)
            }
            ...
            ccxt.PanicOnError(response)
            var items interface{} = ccxt.GetValue(response, "data")
...
func  (this *coinmate) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            __tmp286778 := this.HandleWithdrawTagAndParams(tag, params)
            tag = ccxt.GetValue(__tmp286778, 0)
            params = ccxt.GetValue(__tmp286778, 1)
            ...
            if ccxt.IsTrue(ccxt.IsEqual(method, nil)) {
                var allowedCurrencies interface{} = ccxt.ObjectKeys(methods)
                panic(ccxt.ExchangeError(ccxt.Add(ccxt.Add(this.Id, " withdraw() only allows withdrawing the following currencies: "), ccxt.Join(allowedCurrencies, ", "))))
            }
            var request interface{} = map[string]interface{} {
            ...
            if ccxt.IsTrue(fillResponseFromRequest) {
                ccxt.AddElementToObject(transaction, "amount", amount)
                ccxt.AddElementToObject(transaction, "currency", code)
                ccxt.AddElementToObject(transaction, "address", address)
                ccxt.AddElementToObject(transaction, "tag", tag)
                ccxt.AddElementToObject(transaction, "type", "withdrawal")
                ccxt.AddElementToObject(transaction, "status", "pending")
            }
...
func  (this *coinmate) FetchMyTrades(optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(symbol, nil)) {
                var market interface{} = this.Market(symbol)
                ccxt.AddElementToObject(request, "currencyPair", ccxt.GetValue(market, "id"))
            }
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ccxt.AddElementToObject(request, "timestampFrom", since)
            }
...
func  (this *coinmate) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    takerOrMaker = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(takerOrMaker, "MAKER"))), "maker", "taker")
...
func  (this *coinmate) FetchOpenOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            ch <- this.ParseOrders(ccxt.GetValue(response, "data"), nil, since, limit, extension)
            return nil
...
func  (this *coinmate) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var method interface{} = ccxt.Add("privatePost", this.Capitalize(side))
            var market interface{} = this.Market(symbol)
            var request interface{} = map[string]interface{} {
                "currencyPair": ccxt.GetValue(market, "id"),
            }
            if ccxt.IsTrue(ccxt.IsEqual(typeVar, "market")) {
                if ccxt.IsTrue(ccxt.IsEqual(side, "buy")) {
                    ccxt.AddElementToObject(request, "total", this.AmountToPrecision(symbol, amount)) // amount in fiat
                } else {
                    ccxt.AddElementToObject(request, "amount", this.AmountToPrecision(symbol, amount)) // amount in fiat
                }
                method = ccxt.Add(method, "Instant")
            } else {
                ccxt.AddElementToObject(request, "amount", this.AmountToPrecision(symbol, amount)) // amount in crypto
                ccxt.AddElementToObject(request, "price", this.PriceToPrecision(symbol, price))
                method = ccxt.Add(method, this.Capitalize(typeVar))
            }
...
func  (this *coinmate) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    _ = body
    var url interface{} = ccxt.Add(ccxt.Add(ccxt.GetValue(ccxt.GetValue(this.Urls, "api"), "rest"), "/"), path)
