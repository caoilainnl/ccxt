...
func  (this *coinspot) ParseBalance(response interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsArray(balances)) {
        for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(balances)); i++ {
            var currencies interface{} = ccxt.GetValue(balances, i)
            var currencyIds interface{} = ccxt.ObjectKeys(currencies)
            for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(currencyIds)); j++ {
                var currencyId interface{} = ccxt.GetValue(currencyIds, j)
                var balance interface{} = ccxt.GetValue(currencies, currencyId)
        ...
        for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(currencyIds)); i++ {
            ...
            var code interface{} = this.SafeCurrencyCode(currencyId)
            var account interface{} = this.Account()
            ccxt.AddElementToObject(account, "total", this.SafeString(balances, currencyId))
            ccxt.AddElementToObject(result, code, account)
        }
    }
...
func  (this *coinspot) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                "cointype": ccxt.GetValue(market, "id"),
            }
        
            orderbook:= (<-this.PrivatePostOrders(this.Extend(request, params)))
            ccxt.PanicOnError(orderbook)
        
            ch <- this.ParseOrderBook(orderbook, ccxt.GetValue(market, "symbol"), nil, "buyorders", "sellorders", "rate", "amount")
            return nil
...
func  (this *coinspot) FetchTickers(optionalArgs ...interface{}) <- chan interface{} {
            ...
            var result interface{} = map[string]interface{} {}
            var prices interface{} = this.SafeDict(response, "prices", map[string]interface{} {})
            var ids interface{} = ccxt.ObjectKeys(prices)
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(ids)); i++ {
                ...
                if ccxt.IsTrue(ccxt.GetValue(market, "spot")) {
                    ...
                    var ticker interface{} = ccxt.GetValue(prices, id)
...
func  (this *coinspot) FetchMyTrades(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(buyTrades)); i++ {
                ccxt.AddElementToObject(ccxt.GetValue(buyTrades, i), "side", "buy")
            }
            var sellTrades interface{} = this.SafeList(response, "sellorders", []interface{}{})
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(sellTrades)); i++ {
                ccxt.AddElementToObject(ccxt.GetValue(sellTrades, i), "side", "sell")
            }
...
func  (this *coinspot) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(solddate, nil)) {
        ...
        var feeCost interface{} = ccxt.Precise.StringAdd(audfeeExGst, audGst)
...
func  (this *coinspot) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes5638)
            var method interface{} = ccxt.Add("privatePostMy", this.Capitalize(side))
...
func  (this *coinspot) CancelOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(side, "buy")) && ccxt.IsTrue(!ccxt.IsEqual(side, "sell"))) {
                panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " cancelOrder() requires a side parameter, \"buy\" or \"sell\"")))
            }
