    ...
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            "exact": map[string]interface{} {
                ...
                "ccxt.Account is restricted": ccxt.PermissionDenied,
            },
            "broad": map[string]interface{} {
                "Signature not valid": ccxt.AuthenticationError,
                "overloaded": ccxt.ExchangeNotAvailable,
                "ccxt.Account has insufficient Available ccxt.Balance": ccxt.InsufficientFunds,
                "Service unavailable": ccxt.ExchangeNotAvailable,
                "Server ccxt.Error": ccxt.ExchangeError,
                "Unable to cancel order due to existing state": ccxt.InvalidOrder,
                "We require all new traders to verify": ccxt.PermissionDenied,
            },
        },
...
func  (this *bitmex) FetchCurrencies(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(response)); i++ {
                ...
                for j := 0; ccxt.IsLessThan(j, ccxt.GetArrayLength(chains)); j++ {
                    ...
                    var withdrawalFee interface{} = this.ParseNumber(ccxt.Precise.StringMul(withdrawalFeeRaw, precisionString))
                    ...
                    var active interface{} =             (ccxt.IsTrue(isDepositEnabled) && ccxt.IsTrue(isWithdrawEnabled))
                    if ccxt.IsTrue(isDepositEnabled) {
                        depositEnabled = true
                    }
            ...
            "limits": map[string]interface{} {
                ...
                var currencyActive interface{} = ccxt.IsTrue(currencyEnabled) || ccxt.IsTrue((ccxt.IsTrue(depositEnabled) || ccxt.IsTrue(withdrawEnabled)))
                var minWithdrawalString interface{} = this.SafeString(currency, "minWithdrawalAmount")
                var minWithdrawal interface{} = this.ParseNumber(ccxt.Precise.StringMul(minWithdrawalString, precisionString))
                var maxWithdrawalString interface{} = this.SafeString(currency, "maxWithdrawalAmount")
                var maxWithdrawal interface{} = this.ParseNumber(ccxt.Precise.StringMul(maxWithdrawalString, precisionString))
                var minDepositString interface{} = this.SafeString(currency, "minDepositAmount")
                var minDeposit interface{} = this.ParseNumber(ccxt.Precise.StringMul(minDepositString, precisionString))
                var isCrypto interface{} = ccxt.IsEqual(this.SafeString(currency, "currencyType"), "Crypto")
            ...
            "networks": networks,
            "type": ccxt.Ternary(ccxt.IsTrue(isCrypto), "crypto", "other"),
        })
            }
...
func  (this *bitmex) ConvertFromRealAmount(code interface{}, amount interface{}) interface{}  {
    ...
    var finalAmount interface{} = ccxt.Precise.StringDiv(amountString, precision)
...
func  (this *bitmex) AmountToPrecision(symbol interface{}, amount interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsTrue(ccxt.GetValue(market, "spot")) && !ccxt.IsTrue(oldPrecision)) {
        amount = this.ConvertFromRealAmount(ccxt.GetValue(market, "base"), amount)
    }
...
func  (this *bitmex) ConvertFromRawQuantity(symbol interface{}, rawQuantity interface{}, optionalArgs ...interface{}) interface{}  {
    currencySide := ccxt.GetArg(optionalArgs, 0, "base")
    _ = currencySide
    if ccxt.IsTrue(this.SafeValue(this.Options, "oldPrecision")) {
        return this.ParseNumber(rawQuantity)
    }
    symbol = this.SafeSymbol(symbol)
    var marketExists interface{} = this.InArray(symbol, this.Symbols)
    if !ccxt.IsTrue(marketExists) {
        return this.ParseNumber(rawQuantity)
    }
    var market interface{} = this.Market(symbol)
    if ccxt.IsTrue(ccxt.GetValue(market, "spot")) {
        return this.ParseNumber(this.ConvertToRealAmount(ccxt.GetValue(market, currencySide), rawQuantity))
    }
    return this.ParseNumber(rawQuantity)
}
...
func  (this *bitmex) ParseMarket(market interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(typ, "FFWCSX")) {
        typeVar = "swap"
        swap = true
    } else if ccxt.IsTrue(ccxt.IsEqual(typ, "IFXXXP")) {
        typeVar = "spot"
        spot = true
    } else if ccxt.IsTrue(ccxt.IsEqual(typ, "FFCCSX")) {
        typeVar = "future"
        future = true
    } else if ccxt.IsTrue(ccxt.IsEqual(typ, "FFICSX")) {
    ...
    var linear interface{} = ccxt.Ternary(ccxt.IsTrue(contract), (!ccxt.IsTrue(isInverse) && !ccxt.IsTrue(isQuanto)), nil)
    var status interface{} = this.SafeString(market, "state")
    var active interface{} = ccxt.IsEqual(status, "Open") // Open, Settled, Unlisted
    ...
    if ccxt.IsTrue(spot) {
        ...
        if ccxt.IsTrue(linear) {
            ...
            contractSize = this.ParseNumber(ccxt.Precise.StringDiv("1", multiplierString))
        } else {
            var multiplierString interface{} = ccxt.Precise.StringAbs(this.SafeString(market, "multiplier"))
    ...
    var positionIsQuote interface{} =     (ccxt.IsEqual(position, quote))
    var maxOrderQty interface{} = this.SafeNumber(market, "maxOrderQty")
    var initMargin interface{} = this.SafeString(market, "initMargin", "1")
    var maxLeverage interface{} = this.ParseNumber(ccxt.Precise.StringDiv("1", initMargin))
    ...
    return map[string]interface{} {
        ...
        "limits": map[string]interface{} {
            "leverage": map[string]interface{} {
                "min": ccxt.Ternary(ccxt.IsTrue(contract), this.ParseNumber("1"), nil),
                "max": ccxt.Ternary(ccxt.IsTrue(contract), maxLeverage, nil),
            },
            "amount": map[string]interface{} {
                "min": nil,
                "max": ccxt.Ternary(ccxt.IsTrue(positionIsQuote), nil, maxOrderQty),
            },
            ...
            "cost": map[string]interface{} {
                "min": nil,
                "max": ccxt.Ternary(ccxt.IsTrue(positionIsQuote), maxOrderQty, nil),
            },
        },
        "created": nil,
        "info": market,
...
func  (this *bitmex) ParseBalance(response interface{}) interface{}  {
    ...
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(response)); i++ {
        ...
        ccxt.AddElementToObject(account, "free", this.ConvertToRealAmount(code, free))
        ccxt.AddElementToObject(account, "total", this.ConvertToRealAmount(code, total))
...
func  (this *bitmex) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(response)); i++ {
                ...
                var side interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(ccxt.GetValue(order, "side"), "Sell"))), "asks", "bids")
                ...
                if ccxt.IsTrue(!ccxt.IsEqual(price, nil)) {
                    ...
                    ccxt.AppendToArray(&resultSide, []interface{}{price, amount})
                }
            }
...
func  (this *bitmex) FetchOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(response)
            var numResults interface{} =     ccxt.GetArrayLength(response)
            if ccxt.IsTrue(ccxt.IsEqual(numResults, 1)) {
        
                ch <- ccxt.GetValue(response, 0)
                return nil
            }
            panic(ccxt.OrderNotFound(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, ": The order "), id), " not found.")))
...
func  (this *bitmex) FetchOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes10448)
            var paginate interface{} = false
            __tmp540783 := this.HandleOptionAndParams(params, "fetchOrders", "paginate")
            paginate = ccxt.GetValue(__tmp540783, 0)
            params = ccxt.GetValue(__tmp540783, 1)
            if ccxt.IsTrue(paginate) {
        
                    retRes104819 :=  (<-this.FetchPaginatedCallDynamic("fetchOrders", symbol, since, limit, params, 100))
                    ccxt.PanicOnError(retRes104819)
                    ch <- retRes104819
                    return nil
            }
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ccxt.AddElementToObject(request, "startTime", this.Iso8601(since))
            }
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(until, nil)) {
                ...
                ccxt.AddElementToObject(request, "endTime", this.Iso8601(until))
            }
            request = this.DeepExtend(request, params)
            // why the hassle? urlencode in python is kinda broken for nested dicts.
            // E.g. self.urlencode({"filter": {"open": True}}) will return "filter={'open':+True}"
            // Bitmex doesn't like that. Hence resorting to this hack.
            if ccxt.IsTrue(ccxt.InOp(request, "filter")) {
                ccxt.AddElementToObject(request, "filter", this.Json(ccxt.GetValue(request, "filter")))
            }
...
func  (this *bitmex) FetchMyTrades(optionalArgs ...interface{}) <- chan interface{} {
            ...
            var paginate interface{} = false
            __tmp216908 := this.HandleOptionAndParams(params, "fetchMyTrades", "paginate")
            paginate = ccxt.GetValue(__tmp216908, 0)
            params = ccxt.GetValue(__tmp216908, 1)
            if ccxt.IsTrue(paginate) {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "count", ccxt.MathMin(500, limit))
            }
            var until interface{} = this.SafeInteger2(params, "until", "endTime")
            if ccxt.IsTrue(!ccxt.IsEqual(until, nil)) {
                params = this.Omit(params, []interface{}{"until"})
...
func  (this *bitmex) ParseLedgerEntry(item interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(after, nil)) {
        after = this.ConvertToRealAmount(code, after)
    }
    var before interface{} = this.ParseNumber(ccxt.Precise.StringSub(this.NumberToString(after), this.NumberToString(amount)))
    var direction interface{} = nil
    if ccxt.IsTrue(ccxt.Precise.StringLt(amountString, "0")) {
        direction = "out"
        amount = this.ConvertToRealAmount(code, ccxt.Precise.StringAbs(amountString))
    } else {
        direction = "in"
    }
...
func  (this *bitmex) ParseTransaction(transaction interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(typeVar, "withdrawal")) {
        ...
        addressTo = address
    } else if ccxt.IsTrue(ccxt.IsEqual(typeVar, "deposit")) {
    ...
    var amount interface{} = this.ConvertToRealAmount(ccxt.GetValue(currency, "code"), amountStringAbs)
    var feeCostString interface{} = this.SafeString(transaction, "fee")
    var feeCost interface{} = this.ConvertToRealAmount(ccxt.GetValue(currency, "code"), feeCostString)
    ...
    return map[string]interface{} {
        ...
        "network": this.NetworkIdToCode(this.SafeString(transaction, "network"), ccxt.GetValue(currency, "code")),
        "amount": this.ParseNumber(amount),
        "status": status,
...
func  (this *bitmex) FetchTicker(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsEqual(ticker, nil)) {
                panic(ccxt.BadSymbol(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " fetchTicker() symbol "), symbol), " not found")))
            }
...
func  (this *bitmex) FetchTickers(optionalArgs ...interface{}) <- chan interface{} {
            ...
            var result interface{} = map[string]interface{} {}
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(response)); i++ {
                var ticker interface{} = this.ParseTicker(ccxt.GetValue(response, i))
...
func  (this *bitmex) ParseOHLCV(ohlcv interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var volume interface{} = this.ConvertFromRawQuantity(ccxt.GetValue(market, "symbol"), this.SafeString(ohlcv, "volume"))
...
func  (this *bitmex) FetchOHLCV(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var paginate interface{} = false
            __tmp638579 := this.HandleOptionAndParams(params, "fetchOHLCV", "paginate")
            paginate = ccxt.GetValue(__tmp638579, 0)
            params = ccxt.GetValue(__tmp638579, 1)
            if ccxt.IsTrue(paginate) {
        
                    retRes163119 :=  (<-this.FetchPaginatedCallDeterministic("fetchOHLCV", symbol, since, limit, timeframe, params))
                    ccxt.PanicOnError(retRes163119)
                    ch <- retRes163119
                    return nil
            }
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "count", limit) // default 100, max 500
            }
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ...
                if ccxt.IsTrue(fetchOHLCVOpenTimestamp) {
                    timestamp = this.Sum(timestamp, duration)
                }
                var startTime interface{} = this.Iso8601(timestamp)
                ccxt.AddElementToObject(request, "startTime", startTime) // starting date filter for results
            } else {
                ccxt.AddElementToObject(request, "reverse", true)
            }
            ...
            if ccxt.IsTrue(fetchOHLCVOpenTimestamp) {
                ...
                for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(result)); i++ {
                    ccxt.AddElementToObject(ccxt.GetValue(result, i), 0, ccxt.Subtract(ccxt.GetValue(ccxt.GetValue(result, i), 0), duration))
                }
            }
...
func  (this *bitmex) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    // ccxt.Trade or Funding
    var execType interface{} = this.SafeString(trade, "execType")
    var takerOrMaker interface{} = nil
    if ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(feeCostString, nil)) && ccxt.IsTrue(ccxt.IsEqual(execType, "ccxt.Trade"))) {
        takerOrMaker = ccxt.Ternary(ccxt.IsTrue(ccxt.Precise.StringLt(feeCostString, "0")), "maker", "taker")
    }
    ...
    return this.SafeTrade(map[string]interface{} {
        ...
        "side": side,
        "price": priceString,
        "cost": ccxt.Precise.StringAbs(execCost),
        "amount": amountString,
        "fee": fee,
    }, market)
}
...
func  (this *bitmex) ParseOrder(order interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(marketId, nil)) {
        ...
        isInverse = (ccxt.IsEqual(defaultSubType, "inverse"))
    } else {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(execInst, nil)) {
        postOnly = (ccxt.IsEqual(execInst, "ParticipateDoNotInitiate"))
    }
...
func  (this *bitmex) FetchTrades(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes19428)
            var paginate interface{} = false
            __tmp642681 := this.HandleOptionAndParams(params, "fetchTrades", "paginate")
            paginate = ccxt.GetValue(__tmp642681, 0)
            params = ccxt.GetValue(__tmp642681, 1)
            if ccxt.IsTrue(paginate) {
        
                    retRes194619 :=  (<-this.FetchPaginatedCallDynamic("fetchTrades", symbol, since, limit, params))
                    ccxt.PanicOnError(retRes194619)
                    ch <- retRes194619
                    return nil
            }
            var market interface{} = this.Market(symbol)
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "count", ccxt.MathMin(limit, 1000)) // api maximum 1000
            }
            var until interface{} = this.SafeInteger2(params, "until", "endTime")
            if ccxt.IsTrue(!ccxt.IsEqual(until, nil)) {
                params = this.Omit(params, []interface{}{"until"})
...
func  (this *bitmex) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(reduceOnly, nil)) {
                if ccxt.IsTrue(ccxt.IsTrue((!ccxt.IsTrue(ccxt.GetValue(market, "swap")))) && ccxt.IsTrue((!ccxt.IsTrue(ccxt.GetValue(market, "future"))))) {
                    panic(ccxt.InvalidOrder(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " createOrder() does not support reduceOnly for "), ccxt.GetValue(market, "type")), " orders, reduceOnly orders are supported for swap and future markets only")))
                }
            }
            ...
            if ccxt.IsTrue(ccxt.IsTrue(isTriggerOrder) || ccxt.IsTrue(isTrailingAmountOrder)) {
                var triggerDirection interface{} = this.SafeString(params, "triggerDirection")
                var triggerAbove interface{} =         (ccxt.IsTrue((ccxt.IsEqual(triggerDirection, "ascending"))) || ccxt.IsTrue((ccxt.IsEqual(triggerDirection, "above"))))
                ...
                if ccxt.IsTrue(ccxt.IsEqual(typeVar, "limit")) {
                    if ccxt.IsTrue(ccxt.IsEqual(side, "buy")) {
                        orderType = ccxt.Ternary(ccxt.IsTrue(triggerAbove), "StopLimit", "LimitIfTouched")
                    } else {
                        orderType = ccxt.Ternary(ccxt.IsTrue(triggerAbove), "LimitIfTouched", "StopLimit")
                    }
                } else if ccxt.IsTrue(ccxt.IsEqual(typeVar, "market")) {
                    if ccxt.IsTrue(ccxt.IsEqual(side, "buy")) {
                        orderType = ccxt.Ternary(ccxt.IsTrue(triggerAbove), "Stop", "MarketIfTouched")
                    } else {
                        orderType = ccxt.Ternary(ccxt.IsTrue(triggerAbove), "MarketIfTouched", "Stop")
                    }
                }
                if ccxt.IsTrue(isTrailingAmountOrder) {
                    var isStopSellOrder interface{} = ccxt.IsTrue((ccxt.IsEqual(side, "sell"))) && ccxt.IsTrue((ccxt.IsTrue((ccxt.IsEqual(orderType, "Stop"))) || ccxt.IsTrue((ccxt.IsEqual(orderType, "StopLimit")))))
                    var isBuyIfTouchedOrder interface{} = ccxt.IsTrue((ccxt.IsEqual(side, "buy"))) && ccxt.IsTrue((ccxt.IsTrue((ccxt.IsEqual(orderType, "MarketIfTouched"))) || ccxt.IsTrue((ccxt.IsEqual(orderType, "LimitIfTouched")))))
                    if ccxt.IsTrue(ccxt.IsTrue(isStopSellOrder) || ccxt.IsTrue(isBuyIfTouchedOrder)) {
                        trailingAmount = ccxt.Add("-", trailingAmount)
                    }
                    ccxt.AddElementToObject(request, "pegOffsetValue", this.ParseToNumeric(trailingAmount))
                    ccxt.AddElementToObject(request, "pegPriceType", "TrailingStopPeg")
                } else {
                    if ccxt.IsTrue(ccxt.IsEqual(triggerPrice, nil)) {
                        panic(ccxt.ArgumentsRequired(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " createOrder() requires a triggerPrice parameter for the "), orderType), " order type")))
                    }
                    ccxt.AddElementToObject(request, "stopPx", this.ParseToNumeric(this.PriceToPrecision(symbol, triggerPrice)))
                }
                ccxt.AddElementToObject(request, "ordType", orderType)
                params = this.Omit(params, []interface{}{"triggerPrice", "stopPrice", "stopPx", "triggerDirection", "trailingAmount"})
            }
            if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsTrue((ccxt.IsEqual(orderType, "ccxt.Limit"))) || ccxt.IsTrue((ccxt.IsEqual(orderType, "StopLimit")))) || ccxt.IsTrue((ccxt.IsEqual(orderType, "LimitIfTouched")))) {
...
func  (this *bitmex) EditOrder(id interface{}, symbol interface{}, typeVar interface{}, side interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(origClOrdID, nil)) {
                ccxt.AddElementToObject(request, "origClOrdID", origClOrdID)
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(amount, nil)) {
                var qty interface{} = this.ParseToInt(this.AmountToPrecision(symbol, amount))
                ccxt.AddElementToObject(request, "orderQty", qty)
            }
            if ccxt.IsTrue(!ccxt.IsEqual(price, nil)) {
                ccxt.AddElementToObject(request, "price", price)
            }
            var brokerId interface{} = this.SafeString(this.Options, "brokerId", "CCXT")
            ccxt.AddElementToObject(request, "text", brokerId)
...
func  (this *bitmex) CancelOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(err, nil)) {
                if ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(err, "Unable to cancel order due to existing state"), 0)) {
                    panic(ccxt.OrderNotFound(ccxt.Add(ccxt.Add(this.Id, " cancelOrder() failed: "), err)))
                }
            }
...
func  (this *bitmex) CancelOrders(ids interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes21888)
            // https://github.com/ccxt/ccxt/issues/6507
            var clientOrderId interface{} = this.SafeValue2(params, "clOrdID", "clientOrderId")
            var request interface{} = map[string]interface{} {}
            if ccxt.IsTrue(ccxt.IsEqual(clientOrderId, nil)) {
                ccxt.AddElementToObject(request, "orderID", ids)
            } else {
                ccxt.AddElementToObject(request, "clOrdID", clientOrderId)
                params = this.Omit(params, []interface{}{"clOrdID", "clientOrderId"})
            }
...
func  (this *bitmex) ParsePosition(position interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var marginMode interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(crossMargin, true))), "cross", "isolated")
    var notionalString interface{} = ccxt.Precise.StringAbs(this.SafeString2(position, "foreignNotional", "homeNotional"))
    ...
    var contracts interface{} = this.ParseNumber(ccxt.Precise.StringAbs(this.SafeString(position, "currentQty")))
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(homeNotional, nil)) {
        if ccxt.IsTrue(ccxt.IsEqual(ccxt.GetValue(homeNotional, 0), "-")) {
            side = "short"
        } else {
            side = "long"
        }
...
func  (this *bitmex) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            __tmp554176 := this.HandleWithdrawTagAndParams(tag, params)
            tag = ccxt.GetValue(__tmp554176, 0)
            params = ccxt.GetValue(__tmp554176, 1)
            ...
            __tmp943172 := this.HandleNetworkCodeAndParams(params)
            networkCode = ccxt.GetValue(__tmp943172, 0)
            params = ccxt.GetValue(__tmp943172, 1)
            var request interface{} = map[string]interface{} {
                "currency": ccxt.GetValue(currency, "id"),
                "amount": qty,
                "address": address,
                "network": this.NetworkCodeToId(networkCode, ccxt.GetValue(currency, "code")),
            }
            if ccxt.IsTrue(!ccxt.IsEqual(this.Twofa, nil)) {
                ccxt.AddElementToObject(request, "otpToken", ccxt.Totp(this.Twofa))
            }
...
func  (this *bitmex) FetchFundingRates(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(response)); i++ {
                ...
                if ccxt.IsTrue(swap) {
                    ccxt.AppendToArray(&filteredResponse, item)
                }
            }
            symbols = this.MarketSymbols(symbols)
...
func  (this *bitmex) FetchFundingRateHistory(optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes26968)
            var request interface{} = map[string]interface{} {}
            var market interface{} = nil
            if ccxt.IsTrue(ccxt.InOp(this.Currencies, symbol)) {
                var code interface{} = this.Currency(symbol)
                ccxt.AddElementToObject(request, "symbol", ccxt.GetValue(code, "id"))
            } else if ccxt.IsTrue(!ccxt.IsEqual(symbol, nil)) {
                var splitSymbol interface{} = ccxt.Split(symbol, ":")
                var splitSymbolLength interface{} =         ccxt.GetArrayLength(splitSymbol)
                var timeframes interface{} = []interface{}{"nearest", "daily", "weekly", "monthly", "quarterly", "biquarterly", "perpetual"}
                if ccxt.IsTrue(ccxt.IsTrue((ccxt.IsGreaterThan(splitSymbolLength, 1))) && ccxt.IsTrue(this.InArray(ccxt.GetValue(splitSymbol, 1), timeframes))) {
                    var code interface{} = this.Currency(ccxt.GetValue(splitSymbol, 0))
                    symbol = ccxt.Add(ccxt.Add(ccxt.GetValue(code, "id"), ":"), ccxt.GetValue(splitSymbol, 1))
...
func  (this *bitmex) SetLeverage(leverage interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(ccxt.IsTrue((ccxt.IsLessThan(leverage, 0.01))) || ccxt.IsTrue((ccxt.IsGreaterThan(leverage, 100)))) {
                panic(ccxt.BadRequest(ccxt.Add(this.Id, " leverage should be between 0.01 and 100")))
            }
        
            retRes27828 := (<-this.LoadMarkets())
            ccxt.PanicOnError(retRes27828)
            var market interface{} = this.Market(symbol)
            if ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(ccxt.GetValue(market, "type"), "swap")) && ccxt.IsTrue(!ccxt.IsEqual(ccxt.GetValue(market, "type"), "future"))) {
                panic(ccxt.BadSymbol(ccxt.Add(this.Id, " setLeverage() supports future and swap contracts only")))
            }
            var request interface{} = map[string]interface{} {
                "symbol": ccxt.GetValue(market, "id"),
                "leverage": leverage,
                ...
                ccxt.PanicOnError(retRes279115)
                ch <- retRes279115
                return nil
...
func  (this *bitmex) SetMarginMode(marginMode interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes28128)
            var market interface{} = this.Market(symbol)
            if ccxt.IsTrue(ccxt.IsTrue((!ccxt.IsEqual(ccxt.GetValue(market, "type"), "swap"))) && ccxt.IsTrue((!ccxt.IsEqual(ccxt.GetValue(market, "type"), "future")))) {
                panic(ccxt.BadSymbol(ccxt.Add(this.Id, " setMarginMode() supports swap and future contracts only")))
            }
            var enabled interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(marginMode, "cross"))), false, true)
            var request interface{} = map[string]interface{} {
                "symbol": ccxt.GetValue(market, "id"),
                "enabled": enabled,
            }
                ...
                ccxt.PanicOnError(retRes282215)
                ch <- retRes282215
                return nil
...
func  (this *bitmex) FetchDepositAddress(code interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var networkCode interface{} = nil
            __tmp578610 := this.HandleNetworkCodeAndParams(params)
            networkCode = ccxt.GetValue(__tmp578610, 0)
            params = ccxt.GetValue(__tmp578610, 1)
        ...
        ch <- map[string]interface{} {
                ...
                "currency": code,
                "network": networkCode,
                "address": ccxt.Replace(ccxt.Replace(response, "\"", ""), "\"", ""),
                "tag": nil,
            }
            return nil
...
func  (this *bitmex) ParseDepositWithdrawFee(fee interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    var networksLength interface{} =     ccxt.GetArrayLength(networks)
    var result interface{} = map[string]interface{} {
        "info": fee,
        "withdraw": map[string]interface{} {
            "fee": nil,
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(networksLength, 0)) {
        ...
        for i := 0; ccxt.IsLessThan(i, networksLength); i++ {
            var network interface{} = ccxt.GetValue(networks, i)
            ...
            var withdrawalFee interface{} = this.ParseNumber(ccxt.Precise.StringMul(withdrawalFeeId, precision))
    ...
    "withdraw": map[string]interface{} {
        ...
        "percentage": false,
            ...
            if ccxt.IsTrue(ccxt.IsEqual(networksLength, 1)) {
                ccxt.AddElementToObject(ccxt.GetValue(result, "withdraw"), "fee", withdrawalFee)
...
func  (this *bitmex) FetchDepositWithdrawFees(optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes29358)
        
            assets:= (<-this.PublicGetWalletAssets(params))
            ccxt.PanicOnError(assets)
...
func  (this *bitmex) CalculateRateLimiterCost(api interface{}, method interface{}, path interface{}, params interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(cost, 1)) {
        if ccxt.IsTrue(isAuthenticated) {
            return cost
        } else {
            return 20
        }
...
func  (this *bitmex) FetchLiquidations(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var paginate interface{} = false
            __tmp179723 := this.HandleOptionAndParams(params, "fetchLiquidations", "paginate")
            paginate = ccxt.GetValue(__tmp179723, 0)
            params = ccxt.GetValue(__tmp179723, 1)
            if ccxt.IsTrue(paginate) {
        
                    retRes300019 :=  (<-this.FetchPaginatedCallDynamic("fetchLiquidations", symbol, since, limit, params))
                    ccxt.PanicOnError(retRes300019)
                    ch <- retRes300019
                    return nil
            }
            var market interface{} = this.Market(symbol)
            ...
            __tmp467022 := this.HandleUntilOption("endTime", request, params)
            request = ccxt.GetValue(__tmp467022, 0)
            params = ccxt.GetValue(__tmp467022, 1)
...
func  (this *bitmex) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    _ = body
    var query interface{} = ccxt.Add(ccxt.Add(ccxt.Add("/api/", this.Version), "/"), path)
    if ccxt.IsTrue(ccxt.IsEqual(method, "GET")) {
        if ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(params))) {
            query = ccxt.Add(query, ccxt.Add("?", this.Urlencode(params)))
        }
    } else {
        var format interface{} = this.SafeString(params, "_format")
        if ccxt.IsTrue(!ccxt.IsEqual(format, nil)) {
            query = ccxt.Add(query, ccxt.Add("?", this.Urlencode(map[string]interface{} {
    "_format": format,
})))
    ...
    var isAuthenticated interface{} = this.CheckRequiredCredentials(false)
    if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(api, "private")) || ccxt.IsTrue((ccxt.IsTrue(ccxt.IsEqual(api, "public")) && ccxt.IsTrue(isAuthenticated)))) {
        this.CheckRequiredCredentials()
        var auth interface{} = ccxt.Add(method, query)
        ...
        var stringExpires interface{} = ccxt.ToString(expires)
        auth = ccxt.Add(auth, stringExpires)
        ccxt.AddElementToObject(headers, "api-expires", stringExpires)
        ...
        ccxt.AddElementToObject(headers, "api-signature", this.Hmac(this.Encode(auth), this.Encode(this.Secret), ccxt.Sha256))
    }
    return map[string]interface{} {
        "url": url,
        "method": method,
