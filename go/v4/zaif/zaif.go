    ...
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            "exact": map[string]interface{} {
                "unsupported currency_pair": ccxt.BadRequest,
            },
...
func  (this *zaif) ParseBalance(response interface{}) interface{}  {
    ...
    var currencyIds interface{} = ccxt.ObjectKeys(funds)
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(currencyIds)); i++ {
        ...
        ccxt.AddElementToObject(account, "free", balance)
        ccxt.AddElementToObject(account, "total", balance)
        if ccxt.IsTrue(!ccxt.IsEqual(deposit, nil)) {
            if ccxt.IsTrue(ccxt.InOp(deposit, currencyId)) {
                ccxt.AddElementToObject(account, "total", this.SafeString(deposit, currencyId))
            }
        }
...
func  (this *zaif) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    side = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(side, "bid"))), "buy", "sell")
...
func  (this *zaif) FetchTrades(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            //      ]
            //
            var numTrades interface{} =     ccxt.GetArrayLength(response)
            if ccxt.IsTrue(ccxt.IsEqual(numTrades, 1)) {
                var firstTrade interface{} = ccxt.GetValue(response, 0)
                if !ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(firstTrade))) {
                    response = []interface{}{}
                }
            }
...
func  (this *zaif) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                ...
                "action": ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(side, "buy"))), "bid", "ask"),
                "amount": amount,
                "price": price,
            }
            ...
            ch <- this.SafeOrder(map[string]interface{} {
                "info": response,
                "id": ccxt.ToString(ccxt.GetValue(ccxt.GetValue(response, "return"), "order_id")),
            }, market)
            return nil
...
func  (this *zaif) FetchOpenOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            ch <- this.ParseOrders(ccxt.GetValue(response, "return"), market, since, limit)
            return nil
...
func  (this *zaif) Withdraw(code interface{}, amount interface{}, address interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            params := ccxt.GetArg(optionalArgs, 1, map[string]interface{} {})
            _ = params
            __tmp385119 := this.HandleWithdrawTagAndParams(tag, params)
            tag = ccxt.GetValue(__tmp385119, 0)
            params = ccxt.GetValue(__tmp385119, 1)
            this.CheckAddress(address)
        
            retRes6988 := (<-this.LoadMarkets())
            ccxt.PanicOnError(retRes6988)
            var currency interface{} = this.Currency(code)
            if ccxt.IsTrue(ccxt.IsEqual(code, "JPY")) {
                panic(ccxt.ExchangeError(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " withdraw() does not allow "), code), " withdrawals")))
            }
            var request interface{} = map[string]interface{} {
...
func  (this *zaif) CustomNonce() interface{}  {
    var num interface{} = this.NumberToString(ccxt.Divide(this.Milliseconds(), 1000))
    var nonce interface{} = ccxt.ParseFloat(num)
    return ccxt.ToFixed(nonce, 8)
}
func  (this *zaif) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(api, "public")) {
        url = ccxt.Add(url, ccxt.Add(ccxt.Add(ccxt.Add("api/", this.Version), "/"), this.ImplodeParams(path, params)))
    } else if ccxt.IsTrue(ccxt.IsEqual(api, "fapi")) {
        url = ccxt.Add(url, ccxt.Add(ccxt.Add(ccxt.Add("fapi/", this.Version), "/"), this.ImplodeParams(path, params)))
    } else {
        this.CheckRequiredCredentials()
        if ccxt.IsTrue(ccxt.IsEqual(api, "ecapi")) {
            url = ccxt.Add(url, "ecapi")
        } else if ccxt.IsTrue(ccxt.IsEqual(api, "tlapi")) {
            url = ccxt.Add(url, "tlapi")
        } else {
            url = ccxt.Add(url, "tapi")
        }
