    ...
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            "exact": map[string]interface{} {
                "400": ccxt.BadRequest,
                "409": ccxt.BadSymbol,
                "416": ccxt.InsufficientFunds,
                "417": ccxt.OrderNotFound,
            },
...
func  (this *bitbns) FetchMarkets(optionalArgs ...interface{}) <- chan interface{} {
            ...
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(response)); i++ {
                ...
                var usdt interface{} =         (ccxt.IsEqual(quoteId, "USDT"))
                // INR markets don't need a _INR prefix
                var uppercaseId interface{} = ccxt.Ternary(ccxt.IsTrue(usdt), (ccxt.Add(ccxt.Add(baseId, "_"), quoteId)), baseId)
...
func  (this *bitbns) FetchOrderBook(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(limit, nil)) {
                ccxt.AddElementToObject(request, "limit", limit) // default 100, max 5000, see https://github.com/binance-exchange/binance-official-api-docs/blob/master/rest-api.md#order-book
            }
            ...
            ch <- this.ParseOrderBook(response, ccxt.GetValue(market, "symbol"), timestamp)
            return nil
...
func  (this *bitbns) ParseBalance(response interface{}) interface{}  {
    ...
    for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(keys)); i++ {
        ...
        var parts interface{} = ccxt.Split(key, "availableorder")
        ...
        if ccxt.IsTrue(ccxt.IsGreaterThan(numParts, 1)) {
            ...
            ccxt.AddElementToObject(account, "free", this.SafeString(data, key))
            ccxt.AddElementToObject(account, "used", this.SafeString(data, ccxt.Add("inorder", currencyId)))
            if ccxt.IsTrue(ccxt.IsEqual(currencyId, "Money")) {
                currencyId = "INR"
            }
...
func  (this *bitbns) ParseOrder(order interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(data, "Successfully cancelled the order")) {
        status = "cancelled"
    } else {
...
func  (this *bitbns) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var request interface{} = map[string]interface{} {
                ...
                "symbol": ccxt.GetValue(market, "uppercaseId"),
            ...
            if ccxt.IsTrue(ccxt.IsEqual(typeVar, "limit")) {
                ...
                ccxt.AddElementToObject(request, "market", ccxt.GetValue(market, "quoteId"))
            }
            if ccxt.IsTrue(!ccxt.IsEqual(triggerPrice, nil)) {
                ccxt.AddElementToObject(request, "t_rate", this.PriceToPrecision(symbol, triggerPrice))
            }
            if ccxt.IsTrue(!ccxt.IsEqual(targetRate, nil)) {
                ccxt.AddElementToObject(request, "target_rate", this.PriceToPrecision(symbol, targetRate))
            }
            if ccxt.IsTrue(!ccxt.IsEqual(trailRate, nil)) {
                ccxt.AddElementToObject(request, "trail_rate", this.PriceToPrecision(symbol, trailRate))
            }
...
func  (this *bitbns) CancelOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var tail interface{} = ccxt.Ternary(ccxt.IsTrue(isTrigger), "StopLossOrder", "ccxt.Order")
            var quoteSide interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(ccxt.GetValue(market, "quoteId"), "USDT"))), "usdtcancel", "cancel")
            quoteSide = ccxt.Add(quoteSide, tail)
            ccxt.AddElementToObject(request, "side", quoteSide)
...
func  (this *bitbns) FetchOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(trigger) {
                panic(ccxt.BadRequest(ccxt.Add(this.Id, " fetchOrder cannot fetch stop orders")))
            }
...
func  (this *bitbns) FetchOpenOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            var isTrigger interface{} = this.SafeBool2(params, "trigger", "stop")
            params = this.Omit(params, []interface{}{"trigger", "stop"})
            var quoteSide interface{} = ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(ccxt.GetValue(market, "quoteId"), "USDT"))), "usdtListOpen", "listOpen")
            var request interface{} = map[string]interface{} {
                "symbol": ccxt.GetValue(market, "uppercaseId"),
                "page": 0,
                "side": ccxt.Ternary(ccxt.IsTrue(isTrigger), (ccxt.Add(quoteSide, "StopOrders")), (ccxt.Add(quoteSide, "Orders"))),
            }
...
func  (this *bitbns) ParseTrade(trade interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(side, nil)) {
        if ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(side, "buy"), 0)) {
            side = "buy"
        } else if ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(side, "sell"), 0)) {
            side = "sell"
        }
    }
    var factor interface{} = this.SafeString(trade, "factor")
    var costString interface{} = nil
    if ccxt.IsTrue(!ccxt.IsEqual(factor, nil)) {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(feeCostString, nil)) {
        var feeCurrencyCode interface{} = ccxt.GetValue(market, "quote")
        fee = map[string]interface{} {
            "cost": feeCostString,
...
func  (this *bitbns) FetchMyTrades(optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ccxt.AddElementToObject(request, "since", this.Iso8601(since))
            }
...
func  (this *bitbns) FetchTrades(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.PanicOnError(retRes10218)
            var market interface{} = this.Market(symbol)
            var request interface{} = map[string]interface{} {
                "coin": ccxt.GetValue(market, "baseId"),
                "market": ccxt.GetValue(market, "quoteId"),
            }
...
func  (this *bitbns) ParseTransaction(transaction interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(typeVar, nil)) {
        if ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(typeVar, "deposit"), 0)) {
            typeVar = "deposit"
            status = "ok"
        } else if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(typeVar, "withdraw"), 0)) || ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(expTime, "withdraw"), 0))) {
            typeVar = "withdrawal"
        }
    }
...
func  (this *bitbns) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    api := ccxt.GetArg(optionalArgs, 0, "www")
    _ = api
    ...
    if !ccxt.IsTrue((ccxt.InOp(ccxt.GetValue(urls, "api"), api))) {
        panic(ccxt.ExchangeError(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " does not have a testnet/sandbox URL for "), api), " endpoints")))
    }
    if ccxt.IsTrue(!ccxt.IsEqual(api, "www")) {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(method, "GET")) {
        ...
        ccxt.AddElementToObject(headers, "X-BITBNS-PAYLOAD", payload)
        ccxt.AddElementToObject(headers, "X-BITBNS-SIGNATURE", signature)
...
func  (this *bitbns) HandleErrors(httpCode interface{}, reason interface{}, url interface{}, method interface{}, headers interface{}, body interface{}, response interface{}, requestHeaders interface{}, requestBody interface{}) interface{}  {
    ...
    var err interface{} = ccxt.IsTrue(ccxt.IsTrue((!ccxt.IsEqual(code, nil))) && ccxt.IsTrue((!ccxt.IsEqual(code, "200")))) && ccxt.IsTrue((!ccxt.IsEqual(code, "204")))
