    ...
    return this.DeepExtend(this.Exchange.Describe(), map[string]interface{} {
        ...
        "exceptions": map[string]interface{} {
            "exact": map[string]interface{} {
                "404": ccxt.BadRequest,
                "missing_auth_signature": ccxt.AuthenticationError,
                "order_rejected": ccxt.InvalidOrder,
                "invalid_order_id": ccxt.InvalidOrder,
                "filter_lotsize_maxqty": ccxt.InvalidOrder,
                "filter_notional_min": ccxt.InvalidOrder,
                "failed_index_price_up_multiplier_filter": ccxt.InvalidOrder,
                "no_open_orders": ccxt.InvalidOrder,
                "active_position_not_found": ccxt.InvalidOrder,
                "position_inactive": ccxt.InvalidOrder,
                "invalid_position_id": ccxt.InvalidOrder,
                "Internal server error": ccxt.ExchangeError,
            },
...
func  (this *defx) FetchMarkets(optionalArgs ...interface{}) <- chan interface{} {
            ...
            // }
            //
            var activeMarkets interface{} = this.SafeList(ccxt.GetValue(responses, 0), "data")
            var activeMarketsByType interface{} = this.IndexBy(activeMarkets, "market")
            var marketMetadatas interface{} = this.SafeList(ccxt.GetValue(responses, 1), "data")
            for i := 0; ccxt.IsLessThan(i, ccxt.GetArrayLength(marketMetadatas)); i++ {
                var marketId interface{} = ccxt.GetValue(ccxt.GetValue(marketMetadatas, i), "market")
                var status interface{} = nil
                if ccxt.IsTrue(ccxt.InOp(activeMarketsByType, marketId)) {
                    status = ccxt.GetValue(ccxt.GetValue(activeMarketsByType, marketId), "status")
                }
                ccxt.AddElementToObject(ccxt.GetValue(marketMetadatas, i), "status", status)
            }
...
func  (this *defx) ParseMarket(market interface{}) interface{}  {
    ...
    var symbol interface{} = ccxt.Add(ccxt.Add(ccxt.Add(ccxt.Add(base, "/"), quote), ":"), quote)
    ...
    return map[string]interface{} {
        ...
        "future": false,
        "option": false,
        "active": ccxt.IsEqual(this.SafeString(market, "status", ""), "active"),
        "contract": true,
        "linear": true,
        "inverse": false,
...
func  (this *defx) FetchTickers(optionalArgs ...interface{}) <- chan interface{} {
            ...
            __tmp570292 := this.HandleMarketTypeAndParams("fetchTickers", market, params)
            typeVar = ccxt.GetValue(__tmp570292, 0)
            params = ccxt.GetValue(__tmp570292, 1)
            if ccxt.IsTrue(ccxt.IsEqual(typeVar, "spot")) {
                panic(ccxt.NotSupported(ccxt.Add(ccxt.Add(ccxt.Add(this.Id, " fetchTickers() is not supported for "), typeVar), " markets")))
            }
...
func  (this *defx) FetchOHLCV(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            ccxt.AddElementToObject(request, "endTime", ccxt.Ternary(ccxt.IsTrue((ccxt.IsEqual(until, nil))), this.Milliseconds(), until))
            if ccxt.IsTrue(ccxt.IsEqual(since, nil)) {
                ccxt.AddElementToObject(request, "startTime", 0)
            } else {
                ...
                if ccxt.IsTrue(ccxt.IsEqual(until, nil)) {
                    ...
                    var timeframeInMilliseconds interface{} = ccxt.Multiply(timeframeInSeconds, 1000)
                    var totalTimeframeInMilliseconds interface{} = ccxt.Multiply(limit, timeframeInMilliseconds)
                    ccxt.AddElementToObject(request, "endTime", this.Sum(since, totalTimeframeInMilliseconds))
                }
            }
...
func  (this *defx) CreateOrder(symbol interface{}, typeVar interface{}, side interface{}, amount interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var isLimit interface{} = ccxt.IsEqual(orderType, "LIMIT")
            ...
            if ccxt.IsTrue(ccxt.IsTrue(!ccxt.IsEqual(triggerPrice, nil)) || ccxt.IsTrue(!ccxt.IsEqual(takeProfitPrice, nil))) {
                ccxt.AddElementToObject(request, "workingType", "MARK_PRICE")
            ...
            if ccxt.IsTrue(ccxt.IsTrue(isLimit) && ccxt.IsTrue(!ccxt.IsEqual(price, nil))) {
...
func  (this *defx) ParseOrder(order interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(!ccxt.IsEqual(orderType, nil)) {
        if ccxt.IsTrue(ccxt.IsGreaterThanOrEqual(ccxt.GetIndexOf(orderType, "take_profit"), 0)) {
...
func  (this *defx) CancelOrder(id interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            var isByClientOrder interface{} = !ccxt.IsEqual(clientOrderId, nil)
            if ccxt.IsTrue(isByClientOrder) {
                if ccxt.IsTrue(ccxt.IsEqual(symbol, nil)) {
                    panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " cancelOrder() requires a symbol argument")))
                }
                var market interface{} = this.Market(symbol)
                ccxt.AddElementToObject(request, "orderId", clientOrderId)
                ccxt.AddElementToObject(request, "idType", "clientOrderId")
            ...
            if ccxt.IsTrue(isByClientOrder) {
                ccxt.AddElementToObject(extendParams, "clientOrderId", clientOrderId)
            } else {
...
func  (this *defx) ParsePosition(position interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    market = this.SafeMarket(marketId, market)
    var size interface{} = ccxt.Precise.StringAbs(this.SafeString(position, "quantity"))
...
func  (this *defx) FetchClosedOrders(optionalArgs ...interface{}) <- chan interface{} {
            ...
            var req interface{} = map[string]interface{} {
                ...
                ccxt.PanicOnError(retRes179215)
                ch <- retRes179215
                return nil
...
func  (this *defx) ClosePosition(symbol interface{}, optionalArgs ...interface{}) <- chan interface{} {
            ...
            if ccxt.IsTrue(!ccxt.IsEqual(typeVar, "MARKET")) {
                ...
                if ccxt.IsTrue(ccxt.IsEqual(price, nil)) {
                    panic(ccxt.ArgumentsRequired(ccxt.Add(this.Id, " closePosition() requires a price")))
                }
...
func  (this *defx) FetchLedger(optionalArgs ...interface{}) <- chan interface{} {
            ...
            __tmp355544 := this.HandleOptionAndParams(params, "fetchLedger", "paginate")
            paginate = ccxt.GetValue(__tmp355544, 0)
            params = ccxt.GetValue(__tmp355544, 1)
            if ccxt.IsTrue(paginate) {
        
                    retRes190519 :=  (<-this.FetchPaginatedCallDynamic("fetchLedger", code, since, limit, params))
                    ccxt.PanicOnError(retRes190519)
                    ch <- retRes190519
                    return nil
            }
            var request interface{} = map[string]interface{} {}
            if ccxt.IsTrue(!ccxt.IsEqual(since, nil)) {
                ...
                ccxt.AddElementToObject(request, "start", 0)
            }
            var until interface{} = this.SafeInteger(params, "until")
            if ccxt.IsTrue(!ccxt.IsEqual(until, nil)) {
                params = this.Omit(params, "until")
                ...
                ccxt.AddElementToObject(request, "end", this.Milliseconds())
            }
...
func  (this *defx) Sign(path interface{}, optionalArgs ...interface{}) interface{}  {
    ...
    if ccxt.IsTrue(ccxt.IsEqual(access, "public")) {
        url = ccxt.Add(url, ccxt.Add("open/", pathWithParams))
        ...
        url = ccxt.Add(url, ccxt.Add("auth/", pathWithParams))
        var nonce interface{} = ccxt.ToString(this.Milliseconds())
        var payload interface{} = nonce
        if ccxt.IsTrue(ccxt.IsTrue(ccxt.IsEqual(method, "GET")) || ccxt.IsTrue(ccxt.IsEqual(path, "api/order/{orderId}"))) {
            payload = ccxt.Add(payload, this.Rawencode(params))
            if ccxt.IsTrue(ccxt.GetArrayLength(ccxt.ObjectKeys(params))) {
                url = ccxt.Add(url, ccxt.Add("?", this.Rawencode(params)))
            }
        } else {
        ...
        ccxt.AddElementToObject(headers, "X-DEFX-APIKEY", this.ApiKey)
        ccxt.AddElementToObject(headers, "X-DEFX-TIMESTAMP", nonce)
        ccxt.AddElementToObject(headers, "X-DEFX-SIGNATURE", signature)
    }
    return map[string]interface{} {
        "url": url,
        "method": method,
